...
import { Car, Fuel, Gauge, Calendar, Search, CheckCircle2 } from 'lucide-react'
import Info from './components/Info.jsx'
import LeadForm from './components/LeadForm.jsx'
import FinanceCalculator from './components/FinanceCalculator.jsx'

// ——— CSV Loader (עם עקיפת מטמון) ———
...
export default function App() {
  ...
  // שינוי למספר שלך (בינלאומי, ללא + וללא 0 בתחילת המספר הישראלי)
  const whatsappNumber = '9725XXXXXXXX'
  ...
  useEffect(() => {
    parseCSV('/inventory.csv')
      ...
      .catch((e) => {
        ...
        // Fallback אם אין CSV
        setCars([
          { id:'n1', title:'Hyundai Tucson Premium', year:2025, price:169900, msrp:184900, km:0, gear:'אוטומט', fuel:'היברידי', color:'לבן', delivery_weeks:2, image:'https://images.unsplash.com/photo-1619767886558-efdc259cde1f?q=80&w=1600&auto=format&fit=crop', highlights:['חדש 0 ק״מ','מחיר השקה','ליווי עד מסירה'], sold:false },
        ])
      })
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    return items
  }, [cars, query, fuel, sort, maxPrice])

  const handleWhatsApp = (car) => {
    const text = encodeURIComponent(`שלום, מעוניין ב-${car.title} חדש 0 ק״מ שנת ${car.year} במחיר ${fmt(car.price)}. אשמח להצעת מימון וליווי מלא.`)
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-white/80 border-b">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src="/logo.png?v=1" alt="R&M מוטורס" className="h-8 w-auto" />
            <div>
          ...
          <a className="rounded-2xl px-4 py-2 border" href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noreferrer">
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-8 px-4 py-12 md:py-16 items-center">
          <div>
            <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-3xl md:text-4xl font-extrabold leading-tight">
              רק רכבים <span className="bg-gradient-to-l from-white to-brand.cream bg-clip-text text-transparent">חדשים 0 ק״מ</span> — ברמה הגבוהה ביותר
            </motion.h2>
            <p className="mt-4 text-white/90">
              השירות הטוב ביותר • מחירים מיוחדים מאוד • מימון מותאם אישית • ליווי מלא בכל שלב — לא חותמים עד שמצאנו את הטוב ביותר עבורך.
            </p>
            ...
            <div className="mt-6 flex items-center gap-3">
              <a href="#inventory"><button className="rounded-2xl px-6 py-2 bg-white text-black">למלאי הזמין</button></a>
              <a href="#finance"><button className="rounded-2xl px-6 py-2 border border-white/50">מחשבוני מימון</button></a>
            </div>
          </div>
          <div className="relative flex items-center justify-center">
            <img src="/logo.png?v=1" alt="R&M מוטורס" className="w-full max-w-md h-72 md:h-96 object-contain" />
          </div>
        </div>
      </section>
      ...
      <section id="inventory" className="border-t bg-white">
        <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            {filtered.map(car => (
              <div key={car.id} className="rounded-3xl overflow-hidden shadow-sm border hover:shadow-md transition bg-white">
                <div className="relative">
                  ...
                  {!!car.delivery_weeks && (
                    <div className="absolute top-3 left-3 bg-black/80 text-white px-3 py-1 rounded-full text-xs">אספקה {car.delivery_weeks} שבועות</div>
                  )}
                </div>
                <div className="p-4">
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16 text-neutral-500">אין רכבים זמינים לפי הסינון. נסו להרחיב.</div>
          )}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid md:grid-cols-3 gap-4">
          {[
            ['השירות הטוב ביותר', 'זמינות מלאה, ייעוץ אובייקטיבי, בדיקות ותיאומים עד המפתח — וגם אחריו.'],
            ['מחירים מיוחדים מאוד', 'יחסי יבואנים וספקים שמאפשרים דילים חזקים על חדש 0 ק״מ.'],
            ['מימון מותאם אישית', 'איתור ההצעה המשתלמת — בלי לחץ, עד שמצאנו את הטוב ביותר עבורך.'],
          ].map(([title,desc]) => (
            <div key={title} className="rounded-2xl border bg-white p-5">
              <div className="flex items-center gap-2 text-lg font-bold"><CheckCircle2 size={18}/>{title}</div>
      ...
      <section className="border-t bg-white">
        <div className="max-w-7xl mx-auto px-4 py-10">
          ...
          <p className="text-neutral-600 mt-1">השאירו פרטים ונחזור אליכם עם ההצעה הטובה ביותר עבורכם.</p>
